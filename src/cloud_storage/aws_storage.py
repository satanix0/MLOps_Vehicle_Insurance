...
class SimpleStorageService:
    ...
    data uploads in, and data retrieval from S3 buckets.
    """
    ...
    def get_bucket(self, bucket_name: str) -> Bucket:
        ...
        """
        logging.info(
            "Entered the get_bucket method of SimpleStorageService class")
        try:
            bucket = self.s3_resource.Bucket(bucket_name)
            logging.info(
                "Exited the get_bucket method of SimpleStorageService class")
    ...
    # @staticmethod makes the method independent of the class instance, meaning
    # you can call it directly using the class name without creating an object of the class.
    ...
    def read_object(object_name: str, decode: bool = True, make_readable: bool = False) -> Union[StringIO, str]:
        ...
        Returns:
            Union[StringIO, str]: Returns the content either as a StringIO object or as a decoded string (str), based on the input flags.
        """
        # logging.info("Entered the read_object method of SimpleStorageService class")
        try:
            # Read and convert the byte content into a UTF-8 string. if decode=True
            func = (
                lambda: object_name.get()["Body"].read().decode()
                if decode else object_name.get()["Body"].read()
            )
            
            # Convert to StringIO, which simulates a file-like object in memory, if make_readable=True
            def conv_func(): return StringIO(func()) if make_readable else func()

            # logging.info("Exited the read_object method of SimpleStorageService class")
            # Result from conv_func() is returned. It is either a StringIO object or a string
            return conv_func()
    ...
    def get_file_object(self, filename: str, bucket_name: str) -> Union[List[object], object]:
        ...
        """
        logging.info(
            "Entered the get_file_object method of SimpleStorageService class")
        try:
            bucket = self.get_bucket(bucket_name)
            file_objects = [
    ...
    def create_folder(self, folder_name: str, bucket_name: str) -> None:
        ...
        """
        logging.info(
            "Entered the create_folder method of SimpleStorageService class")
        try:
    ...
    def upload_file(self, from_filename: str, to_filename: str, bucket_name: str, remove: bool = True):
        ...
        """
        logging.info(
            "Entered the upload_file method of SimpleStorageService class")
        try:
            logging.info(
                f"Uploading {from_filename} to {to_filename} in {bucket_name}")
            self.s3_resource.meta.client.upload_file(
                from_filename, bucket_name, to_filename)
            logging.info(
                f"Uploaded {from_filename} to {to_filename} in {bucket_name}")

            # Delete the local file if remove is True
            if remove:
                os.remove(from_filename)
                logging.info(
                    f"Removed local file {from_filename} after upload")
            logging.info(
                "Exited the upload_file method of SimpleStorageService class")
    ...
    def read_csv(self, filename: str, bucket_name: str) -> DataFrame:
        ...
        """
        logging.info(
            "Entered the read_csv method of SimpleStorageService class")
        try:
            csv_obj = self.get_file_object(filename, bucket_name)
            df = self.get_df_from_object(csv_obj)
            logging.info(
                "Exited the read_csv method of SimpleStorageService class")
